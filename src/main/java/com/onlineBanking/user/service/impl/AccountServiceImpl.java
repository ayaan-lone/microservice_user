...
public class AccountServiceImpl implements AccountService {
	...
	public void createAccount(CreateAccountRequestDto createAccountRequestDto) throws UserApplicationException {
		...
		Optional<Users> userOptional = userRepository.findById(createAccountRequestDto.getUserId());

		if (!userOptional.isPresent()) {
			throw new UserApplicationException(HttpStatus.NOT_FOUND, ConstantUtil.USER_NOT_FOUND);
		}

		String url = ConstantUtil.CREATE_ACCOUNT_API_URL;
		HttpEntity<CreateAccountRequestDto> requestEntity = new HttpEntity<>(createAccountRequestDto);
